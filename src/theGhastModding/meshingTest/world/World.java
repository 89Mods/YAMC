...
public class World implements Runnable {
	...
	private Map<Vector2i, Chunk[]> allChunks;
	private int width;
	private int height;
	private int depth;
	...
	public boolean setBlock(int x, int y, int z, int block, boolean parseLight) {
		...
		removeLight(x, y, z, getAbsoluteSunLightLevel(x, y, z), true);
	...
	private synchronized void parseLightQueue(boolean sunlight) {
		...
		while(!lightQueue.isEmpty()) {
			LightSource ls = lightQueue.remove(lightQueue.size() - 1);
			level = ls.level;
			x = ls.x;
			y = ls.y;
			z = ls.z;
	...
	public synchronized void removeLight(int x, int y, int z, int originalLevel, boolean sunlight) {
		...
		for(int i = 0; i < tempLightQueue.size(); i++) {
			if(tempLightQueue.get(i).getCurrLevel(sunlight) >= tempLightQueue.get(i).level) {
				lightQueue.add(tempLightQueue.get(i));
			}
		}
	...
	private synchronized void unloadChunk(Vector2i v) {
		Chunk[] cs = allChunks.remove(v);
		try {
	...
	public void startGenerateSpawnChunks() {
			...
			while(running) {
				...
				if(System.currentTimeMillis() - debugTime >= 1000 && playerx != -1 && playery != -1 && playerz != -1 && playerx != prevplayerx && playerz != prevplayerz) {
					...
					for(Chunk[] c:getAllLoadedChunks()) {
						...
						if(c[0].getChunkx() > (maxx + 3) || c[0].getChunkz() > (maxz + 3) || c[0].getChunkx() < (minx - 3) || c[0].getChunkz() < (minz - 3)) {
